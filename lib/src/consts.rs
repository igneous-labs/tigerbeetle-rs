use tigerbeetle_unofficial_sys::{tb_account_t, tb_transfer_t};
...
pub const MAX_MSG_BYTES: usize = (1024 * 1024) - 256; // TODO: this size (~1MB) causes stack overflow on default settings, so we're boxing all ResponseBufs if they exceed some size for now
